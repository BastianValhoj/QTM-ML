...
if rank == 0:    
    ...
    HAMS_DIR = WORK_DIR / "bilayer_data" / "ham" 
    if STACK.upper() == "AA":
        OUT_DIR = HAMS_DIR / "AA_stack"
    elif STACK.upper() == "AB": 
        OUT_DIR = HAMS_DIR / "AB_stack"
    else: raise ValueError("STACK must be either AA or AB")
    
    OUT_DIR = OUT_DIR / f"NC{NC}-N{N}_to_{NT}"
...
WHICH = comm.bcast(WHICH, root=0)
ANGLES = comm.bcast(ANGLES, root=0)
MU_TOP = comm.bcast(MU_TOP, root=0)
MU_BOTTOM = comm.bcast(MU_BOTTOM, root=0)
INTER_LAYER_DIST = comm.bcast(INTER_LAYER_DIST, root=0)
OUT_DIR = comm.bcast(OUT_DIR, root=0)
DATA_DIR = comm.bcast(DATA_DIR, root=0)

comm.Barrier()

# distribute angles over ranks
local_idx = np.array_split(np.arange(len(ANGLES)), size)[rank]
print(f"Rank {rank} handles {len(local_idx)} angles", flush=True)
...
def stack_device(data_path, which=("bottom", "bottom"), d=3.35, angle=None):
    ...
    if STACK == "AB":
        geom_top = geom_top.translate([0, 1.42, 0]) # move along y to make A-B stacking
    elif STACK == "AA":
        pass
    else: raise ValueError("STACK must be either AB or AA (default, no change)")
    
    # displace layer from bottom by inter-layer distance, d
...
def main():
    if rank == 0:
        ...
        geom_bilayer_no_rot = None
        idx_dict = None
        idx_top = None
    geom_bilayer_no_rot = comm.bcast(geom_bilayer_no_rot, root=0)
    idx_dict = comm.bcast(idx_dict, root=0)
    idx_top = comm.bcast(idx_top, root=0)
    
    local_results = []
    for idx in tqdm(local_idx, desc=f"Angles"):
        angle = ANGLES[idx]
        geom_bilayer = geom_bilayer_no_rot.rotate(
            [angle, "z"],           # rotate by `angle` around axis [0,0,1] (z)
        ...
        local_results.append((angle, Ham_bilayer))
    ...
    if rank == 0:
        for results in all_results:
            for a, ham in results:
                ham_file_path = OUT_DIR / f"angle-{a}"
                ham_file_path.mkdir(exist_ok=True, parents=True)
                ham.write(ham_file_path / "Ham_bilayer.nc")
                print(f"Hamiltonian saved to `{ham_file_path / "Ham_bilayer.nc"}`", flush=True)
        
        Htb.write(OUT_DIR / "Ham_bi_no_coupling.nc")  # Htb from last iteration, same for all angles
        ...
        print(f"### Saved idx to npz : `{OUT_DIR / 'params.npz'}`", flush=True)
