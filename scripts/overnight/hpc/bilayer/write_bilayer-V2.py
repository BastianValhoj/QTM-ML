...
def remove_edge_coupling(H, idx_top, idx_bot, shift=0):
    ...
    is_bot = np.zerps(shape=Ham.no, dtype=bool)
...
def main(d=3.35, v=0):
    ...
    all_idx_top = list(idx_dict["elec_top"]) + list(idx_dict["device_top"])
    all_idx_bottom = list(idx_dict["elec_bottom"]) + list(idx_dict["elec_bottom"])
    # remove edge region
    Ham_bilayer = remove_edge_coupling(Ham_bilayer,
                                       idx_top=all_idx_top,
                                       idx_bot=all_idx_bottom,
                                       shift=SHIFT)
